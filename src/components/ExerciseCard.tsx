...
const GRID = 'grid grid-cols-[2rem_minmax(0,1fr)_minmax(0,1.1fr)_minmax(0,1.1fr)_2.5rem] items-center gap-2';
...
export function ExerciseCard({
  ...
  const sub = [muscleLabel(exercise.targetMuscle, t), exercise.repRange && exercise.repRange !== '-' ? exercise.repRange : null]
    .filter(Boolean)
    .join(' · ');
  ...
  return (
    <section className="card !p-0 overflow-hidden">
      ...
      <header className="flex items-center justify-between gap-2 px-4 pb-3 pt-4">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="truncate font-display text-base font-semibold tracking-[-0.01em]">{exercise.name}</p>
            <p className="truncate font-mono text-[11.5px] text-earth-muted">{sub}</p>
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-1">
          <button type="button" onClick={() => setShowNotes((v) => !v)} className="icon-btn h-8 w-8" aria-label={t('workout.notes')}>
            <Icon name="info" size={15} />
          </button>
          <button type="button" onClick={onVideo} className="icon-btn h-8 w-8" aria-label={t('workout.watchVideo')}>
            <Icon name="video" size={15} />
          </button>
          {onRemoveExercise && (
            <button type="button" onClick={onRemoveExercise} className="icon-btn h-8 w-8" aria-label={t('common.delete')}>
      ...
      {showNotes && (loc(exercise.notes) || (exercise.tempo && exercise.tempo !== '-')) && (
        <div className="mx-4 mb-3 rounded-xl border border-line bg-surface-raised p-3 text-sm text-earth-muted">
          ...
          {exercise.tempo && exercise.tempo !== '-' && (
            <p className="mt-1 font-mono text-[11px] text-earth-subtle">{t('workout.tempo')} {exercise.tempo}</p>
          )}
        </div>
      )}
      ...
      <div className="px-4 pb-4">
        ...
        <div className={`${GRID} px-1 pb-1`}>
          <span className="font-mono text-[10px] uppercase tracking-[0.08em] text-earth-subtle">{t('workout.set')}</span>
          <span className="font-mono text-[10px] uppercase tracking-[0.08em] text-earth-subtle">{t('gt.prev')}</span>
          <span className="text-center font-mono text-[10px] uppercase tracking-[0.08em] text-earth-subtle">{t('gt.kgCol')}</span>
          <span className="text-center font-mono text-[10px] uppercase tracking-[0.08em] text-earth-subtle">{t('gt.repsCol')}</span>
          <span />
        </div>
        ...
        <ul className="space-y-1">
          {log.sets.map((set) => {
            ...
            return (
              ...
              >
                <span className={`font-mono text-sm font-medium ${set.done ? 'text-brand' : isWarm ? 'text-warn' : 'text-earth-muted'}`}>
                  {label}
                </span>
                <span className="truncate font-mono text-[11.5px] text-earth-subtle">{prevLabel}</span>
                <input
                  inputMode="decimal"
                  ...
                  onChange={(e) => onUpdateSet(set.setIndex, { weightKg: e.target.value === '' ? null : Number(e.target.value) })}
                  className={cell}
                />
                <input
                  inputMode="numeric"
                  ...
                  onFocus={(e) => e.currentTarget.select()}
                  onChange={(e) => onUpdateSet(set.setIndex, { actualReps: e.target.value === '' ? null : Number(e.target.value) })}
                  className={cell}
                />
                <button
                  type="button"
                  ...
                  className={`flex h-10 w-10 items-center justify-center rounded-[10px] border transition-transform active:scale-90 ${
                    set.done ? 'border-success bg-success text-white' : 'border-line bg-surface-raised text-earth-subtle'
                  }`}
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          {log.sets.length > 1 && (
            <button
              type="button"
              onClick={() => onRemoveSet(log.sets[log.sets.length - 1].setIndex)}
